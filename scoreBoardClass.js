class top_generic_score {
    ...
    show() {
        ...
        L_HUD.push();
        L_HUD.translate(this.x + txtPixel * 15, this.y - txtPixel * 3);
        //L_HUD.rotate(-50);
        L_HUD.image(img_align, 0, 0, txtPixel * 3, txtPixel * 3);
        L_HUD.pop();
...
class top_ghost_score {
    ...
    update() {
        for (let i = 0; i < madot.length; i++) {
            this.arr[i] = { name: madot[i].name, scoreItem: madot[i].ghostDuration, color: madot[i].colorINIT, Index: i };
        }
        this.arr.sort((firstItem, secondItem) => firstItem.scoreItem - secondItem.scoreItem);
        reverse(this.arr);
        this.sizeY = txtPixel * this.arr.length * 1.5 + 1.6 * txtPixel;
